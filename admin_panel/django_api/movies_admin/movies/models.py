...
class Review(UUIDMixin, TimeStampedMixin):
    ...
    author = models.ForeignKey("profiles.Profile", on_delete=models.CASCADE, related_name='profile_reviews')
    film_work = models.ForeignKey('FilmWork', on_delete=models.CASCADE)
...
class Comment(UUIDMixin, TimeStampedMixin):
    ...
    author = models.ForeignKey("profiles.Profile", on_delete=models.CASCADE, related_name='profile_comments')
    body = models.TextField()
    ...
    def __str__(self):
        return 'Comment by {} on {}'.format(self.author, self.review)
